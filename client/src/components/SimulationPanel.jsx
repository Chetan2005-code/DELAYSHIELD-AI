...
import { runSimulationBatch } from "../services/api";
...
export default function SimulationPanel() {
  ...
  function addScenario() {
    setScenarios((prev) => [...prev, { id: nextId++, traffic: "", delay: "" }]);
  }
  ...
  async function runSimulation() {
    ...
    const builtScenarios = scenarios.map((scenario, index) => ({
      label: `Scenario ${index + 1}`,
      traffic:
        scenario.traffic !== ""
          ? stressMode
            ? Math.round(scenario.traffic * 1.5)
            : scenario.traffic
          : stressMode
            ? Math.round(baseInput.traffic * 1.5)
            : baseInput.traffic,
    ...
    let data;
    try {
      data = await runSimulationBatch({ baseInput, scenarios: builtScenarios });
    } catch {
    ...
    <div className="max-w-[720px] py-4 md:py-6 mx-auto w-full px-2 sm:px-0">
      <div className="flex items-start justify-between mb-7 gap-4">
        ...
        <label className="flex items-center gap-2 cursor-pointer shrink-0">
          ...
          <span className="text-xs font-medium text-slate-500 whitespace-nowrap">Stress test (x1.5)</span>
        </label>
      </div>
      ...
      <div className="glass-panel p-4 sm:p-5 mb-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Field label="Traffic">
            <input
              className="w-full bg-slate-50 border border-slate-200 rounded-md px-3 py-1.5 text-sm text-blue-950 focus:outline-none focus:border-blue-400"
              type="number"
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="text-xs px-3 py-1.5 cursor-pointer rounded-lg border border-slate-300 bg-white text-blue-950 hover:bg-slate-50 transition-colors" onClick={addScenario}>
          + Add scenario
        </button>
      </div>
        ...
        {scenarios.map((sc, i) => (
          <div key={sc.id} className="glass-panel p-4 pb-5 !border-l-4 !border-l-blue-400 !border-y-0 !border-r-0 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              {scenarios.length > 1 && (
                <button className="text-[11px] text-slate-400 hover:text-red-500 bg-transparent border-none cursor-pointer" onClick={() => removeScenario(sc.id)}>
                  Remove
                </button>
              )}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Traffic override">
                <input
                  ...
                  value={sc.traffic}
                  onChange={(e) => updateScenario(sc.id, "traffic", e.target.value)}
                  min={0}
                  max={300}
                />
              </Field>
              <Field label="Delay override (ms)">
                <input
                  ...
                  value={sc.delay}
                  onChange={(e) => updateScenario(sc.id, "delay", e.target.value)}
                  min={0}
                  max={500}
                />
              </Field>
